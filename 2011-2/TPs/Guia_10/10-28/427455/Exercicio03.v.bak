...
module RAM4x8(output[7:0] outp , input[7:0] inp, input rw, input[1:0] addr, input clk , input clr);
wire[3:0] x;
wire[7:0] i0;
wire[7:0] i1;
wire[7:0] i2;
wire[7:0] i3;
dmx d1(x,1,addr);
RAM1x8 p0(i0,inp,rw,x[3],clk,clr);
RAM1x8 p1(i1,inp,rw,x[2],clk,clr);
RAM1x8 p2(i2,inp,rw,x[1],clk,clr);
RAM1x8 p3(i3,inp,rw,x[0],clk,clr);
...
reg clk;
reg clr;
RAM4x8 p1(outp,inp,rw,addr,clk,clr);
initial
begin
// initial values
	addr = 2'b00;
